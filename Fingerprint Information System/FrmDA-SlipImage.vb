...
Public Class frmDASlipImageDisplayer
    Dim DAPictureFile As String
    Dim CPPictureFile As String
    Dim ZoomDA As Boolean = False
    Dim ZoomCP As Boolean = False
    Dim DAViewMode As iViewCore.PictureBox.EViewMode
    ...
    Private Sub FormLoadEvents(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        DAViewMode = iViewCore.PictureBox.EViewMode.FitWidth
...
#Region "LOAD DA SLIP"
    ...
    Public Sub LoadDASlipPicture(ByVal PicFile As String, ByVal Title As String)
        On Error Resume Next
        Me.Cursor = Cursors.WaitCursor
        DAPictureFile = PicFile
        EnableOrDisableDASlipButtons()
        Me.picDASlip.ClearImage()
        Me.sldrZoomDASlip.Value = Me.sldrZoomDASlip.Minimum
        Me.lblSlNumber.Text = "DA Slip No. " & Title
        ...
        ZoomDA = True
        EnableOrDisableDAZoomButtons()
        Me.picDASlip.ViewMode = DAViewMode
    End Sub
    ...
    Private Sub ShowPreviousDASlipImage() Handles btnPrevious.Click
        On Error Resume Next
        frmMainInterface.DARegisterBindingSource.MovePrevious()
        DAPictureFile = frmMainInterface.DADatagrid.CurrentRow.Cells(15).Value.ToString()
        If FileIO.FileSystem.FileExists(DAPictureFile) = False Then
            Dim FileName As String = "DANo." & frmMainInterface.DADatagrid.SelectedCells(0).Value.ToString & ".jpeg"
            FileName = Strings.Replace(FileName, "/", "-")
            Dim yr As String = Year(frmMainInterface.DADatagrid.SelectedCells(2).Value)
            DAPictureFile = FPImageImportLocation & "\DA Slips\" & yr & "\" & FileName
        End If
        Dim idno As String = frmMainInterface.DADatagrid.CurrentRow.Cells(0).Value.ToString()
        Dim name As String = frmMainInterface.DADatagrid.CurrentRow.Cells(6).Value.ToString()
        Dim aliasname As String = frmMainInterface.DADatagrid.CurrentRow.Cells(7).Value.ToString()

        LoadDASlipPicture(DAPictureFile, idno & "  -   " & name & IIf(aliasname <> vbNullString, " @ " & aliasname, ""))

    End Sub

    Private Sub ShowNextDASlipImage() Handles btnNext.Click
        On Error Resume Next
        frmMainInterface.DARegisterBindingSource.MoveNext()
        DAPictureFile = frmMainInterface.DADatagrid.CurrentRow.Cells(15).Value.ToString()
        If FileIO.FileSystem.FileExists(DAPictureFile) = False Then
            Dim FileName As String = "DANo." & frmMainInterface.DADatagrid.SelectedCells(0).Value.ToString & ".jpeg"
            FileName = Strings.Replace(FileName, "/", "-")
...
#Region "ENABLE DASLIP BUTTONS"
    ...
    Sub EnableOrDisableDASlipButtons()
        On Error Resume Next
        If frmMainInterface.DARegisterBindingSource.Position = frmMainInterface.DARegisterBindingSource.Count - 1 Then
            Me.btnNext.Enabled = False
        Else
            Me.btnNext.Enabled = True
        End If

        If frmMainInterface.DARegisterBindingSource.Position = 0 Then
    ...
    Private Sub EnableOrDisableDAZoomButtons()
        Me.btnPrintImage.Enabled = ZoomDA
        Me.btnZoomInDASlip.Enabled = ZoomDA
        Me.btnZoomOutDASlip.Enabled = ZoomDA
        Me.sldrZoomDASlip.Enabled = ZoomDA
        Me.btnDAViewMode.Enabled = ZoomDA
    End Sub
#End Region
    ...
    Private Sub SetViewMode(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDAFitHeight.Click, btnDAFitImage.Click, btnDAFitWidth.Click, btnDAFullSize.Click, btnCPFitHeight.Click, btnCPFitImage.Click, btnCPFitWidth.Click, btnCPFullSize.Click, btnCPActualSizeContext.Click, btnCPFitHeightContext.Click, btnCPFitImageContext.Click, btnCPFitWidthContext.Click, btnDAActualSizeContext.Click, btnDAFitHeightContext.Click, btnDAFitSizeContext.Click, btnDAFitWidthContext.Click
        ...
        Select Case ctrl.Name
            ...
            Case btnDAFitHeight.Name
                DAViewMode = iViewCore.PictureBox.EViewMode.FitHeight
                Me.picDASlip.ViewMode = DAViewMode
            Case btnDAFitImage.Name
                DAViewMode = iViewCore.PictureBox.EViewMode.FitImage
                Me.picDASlip.ViewMode = DAViewMode
            Case btnDAFitWidth.Name
                DAViewMode = iViewCore.PictureBox.EViewMode.FitWidth
                Me.picDASlip.ViewMode = DAViewMode
            Case btnDAFullSize.Name
                DAViewMode = iViewCore.PictureBox.EViewMode.FullSize
                Me.picDASlip.ViewMode = DAViewMode
    ...
    Private Sub ResetImages(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDAReset.Click, btnCPReset.Click
        ...
        Select Case ctrl.Name
            Case btnDAReset.Name
                Me.picDASlip.Image = New Bitmap(DAPictureFile)
