...
Public Class frmIDSlipImageDisplayer
...
#Region "LOAD DA SLIP"
    ...
    Public Sub LoadDASlipPicture(ByVal PicFile As String, ByVal Title As String)
        ...
        Me.lblSlNumber.Text = "Identified Slip No. " & Title
    ...
    Private Sub ShowPreviousDASlipImage() Handles btnPrevious.Click
        On Error Resume Next
        frmMainInterface.IDRegisterBindingSource.MovePrevious()
        DAPictureFile = frmMainInterface.IDDatagrid.CurrentRow.Cells(15).Value.ToString()
        If FileIO.FileSystem.FileExists(DAPictureFile) = False Then
            Dim FileName As String = "IDNo." & Format(CInt(frmMainInterface.IDDatagrid.SelectedCells(0).Value), "0000") & ".jpeg"
            DAPictureFile = FPImageImportLocation & "\Identified Slips\" & FileName
        End If

        Dim idno As String = frmMainInterface.IDDatagrid.CurrentRow.Cells(0).Value.ToString()
        Dim name As String = frmMainInterface.IDDatagrid.CurrentRow.Cells(5).Value.ToString()
        Dim aliasname As String = frmMainInterface.IDDatagrid.CurrentRow.Cells(6).Value.ToString()
    ...
    Private Sub ShowNextDASlipImage() Handles btnNext.Click
        On Error Resume Next
        frmMainInterface.IDRegisterBindingSource.MoveNext()
        DAPictureFile = frmMainInterface.IDDatagrid.CurrentRow.Cells(15).Value.ToString()
        If FileIO.FileSystem.FileExists(DAPictureFile) = False Then
            Dim FileName As String = "IDNo." & Format(CInt(frmMainInterface.IDDatagrid.SelectedCells(0).Value), "0000") & ".jpeg"
            DAPictureFile = FPImageImportLocation & "\Identified Slips\" & FileName
...
#Region "ENABLE DASLIP BUTTONS"
    ...
    Sub EnableOrDisableDASlipButtons()
        On Error Resume Next
        If frmMainInterface.IDRegisterBindingSource.Position = frmMainInterface.IDRegisterBindingSource.Count - 1 Then
            Me.btnNext.Enabled = False
        Else
            Me.btnNext.Enabled = True
        End If

        If frmMainInterface.IDRegisterBindingSource.Position = 0 Then
