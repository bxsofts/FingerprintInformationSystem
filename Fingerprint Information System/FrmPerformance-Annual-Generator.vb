...
#Region "LOAD FORM"
    ...
    Private Sub LoadForm() Handles MyBase.Load
        ...
        Dim chkbox As String = My.Computer.Registry.GetValue(strGeneralSettingsPath, "chkAPStatement", 1)
    ...
    Private Sub SaveCheckBox(sender As Object, e As EventArgs) Handles chkiAPS.Click, chkStatement.Click
        Try
            ...
            My.Computer.Registry.SetValue(strGeneralSettingsPath, "chkAPStatement", x, Microsoft.Win32.RegistryValueKind.String)
...
#Region "GENERATE STATEMENT"
    ...
    Private Sub LoadPerformanceFromSavedFile(SavedFileName As String)
        Try
            ...
            For i = 2 To 23
                For j = 3 To 8
                    Me.DataGridViewX1.Rows(i - 2).Cells(j - 1).Value = wdTbl.Cell(i, j).Range.Text.Trim(ChrW(7)).Trim().Replace("` ", "Rs.").Replace("`", "Rs.")
                Next
            Next
    ...
    Private Sub CalculateTotalValues() Handles DataGridViewX1.CellEndEdit
        ...
        For i = 0 To 20
            sum = Val(Me.DataGridViewX1.Rows(i).Cells(2).Value.ToString) + Val(Me.DataGridViewX1.Rows(i).Cells(3).Value.ToString) + Val(Me.DataGridViewX1.Rows(i).Cells(4).Value.ToString) + Val(Me.DataGridViewX1.Rows(i).Cells(5).Value.ToString)
        ...
        Dim v1 = Me.DataGridViewX1.Rows(21).Cells(2).Value.ToString.Replace("Rs.", "").Replace("/-", "")
        Dim v2 = Me.DataGridViewX1.Rows(21).Cells(3).Value.ToString.Replace("Rs.", "").Replace("/-", "")
        Dim v3 = Me.DataGridViewX1.Rows(21).Cells(4).Value.ToString.Replace("Rs.", "").Replace("/-", "")
        Dim v4 = Me.DataGridViewX1.Rows(21).Cells(5).Value.ToString.Replace("Rs.", "").Replace("/-", "")
        Me.DataGridViewX1.Rows(21).Cells(6).Value = "Rs." & Val(v1) + Val(v2) + Val(v3) + Val(v4) & "/-"
